...
def saveWeightMapPNG(pklPath, pngPath=None):
    if pngPath is None:
        pngPath = pklPath.replace(".pkl", "_weight.png")

    with open(pklPath, "rb") as f:
        data = pickle.load(f)

    if "weight_map" not in data:
        print("No weight_map found in", pklPath)
        return
    ...
    img = data["cxr_img"][0].cpu().detach().numpy()

    plt.figure(figsize=(8, 8))
    plt.imshow(img, cmap="gray")
    plt.imshow(weight_map, cmap="viridis", alpha=0.45, vmin=0.0, vmax=1.0)
    plt.axis("off")
