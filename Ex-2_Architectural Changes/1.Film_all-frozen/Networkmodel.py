...
#  FiLMPatchNet
#
#  Identical pipeline to ScalePatchNet, with one insertion between the
#  frozen backbone and the classifier:
#
#    Patch
#      │
#    Frozen EfficientNetV2-S  (features → avgpool → flatten)
#      │
#    1280-dim feature vector
#      │
#    FiLM:  modulated = γ ⊙ feature + β
#           γ ∈ ℝ^1280  (learnable, init = 1)
#           β ∈ ℝ^1280  (learnable, init = 0)
#      │
#    Linear(1280 → 14)  ← weights loaded from ScalePatchNet checkpoint
#      │
#    14 per-patch logits
#      │
...
#
#  What is trainable:
#    - film_gamma  (1280 scalars)
#    - film_beta   (1280 scalars)
#    - classifier  (Linear 1280→14)
#
#  What is frozen:
#    - eff_features  (all EfficientNet feature blocks)
#    - eff_avgpool   (AdaptiveAvgPool2d)
#
#  At initialisation γ=1 and β=0, so the network output is identical to
#  ScalePatchNet.  Training then lets the FiLM parameters shift and scale
#  each feature dimension to better serve the classifier, without touching
...
class FiLMPatchNet(nn.Module):
    ...
    def __init__(self, patchSize, outFeatures=14):
        ...
        #Version 2 -- remove the following code to unfreze the classifier- 
    ...
    def extract_features(self, patches):
        """
        Run patches through the frozen backbone following EfficientNet's
        own forward sequence:  features → avgpool → flatten → 1280-dim vector.

        Call this inside torch.no_grad() (as _run_patches does) to avoid
        storing activations for a gradient that will never be computed.

        Args:
            patches: (N, 1, patchSize, patchSize)
        Returns:
            features: (N, 1280)
        """
    ...
    def _run_patches(self, x):
        """
        Unfold x into patches, run frozen backbone → FiLM → classifier.

        Returns per-patch logits with shape (B, P, 14).
        """
        B = x.size(0)
    ...
    def forward(self, x):
        """Global logits via mean pooling over patches — same as ScalePatchNet."""
        return self._run_patches(x).mean(dim=1)   # (B, outFeatures)

    def forwardRawPatches(self, x):
        """Per-patch logits — same shape and semantics as ScalePatchNet."""
        return self._run_patches(x)               # (B, P, outFeatures)

    def forwardScaledPatches(self, x):
        """
        Per-patch logits scaled by global sigmoid probability — same
        semantics as ScalePatchNet.forwardScaledPatches(), kept for
        drop-in compatibility with evalClassification.py.
        """
    ...
    def from_scalepatchnet_checkpoint(
        ...
        """
        Build a FiLMPatchNet and load the EfficientNet backbone + the
        pretrained CheXpert classifier from a ScalePatchNet *_weights.pt file.

        Key translation:
          baseBackbone.features.*         →  eff_features.*
          baseBackbone.avgpool.*          →  eff_avgpool.*
          baseBackbone.classifier.1.weight → classifier.weight
          baseBackbone.classifier.1.bias   → classifier.bias

        film_gamma and film_beta remain at their default γ=1, β=0.

        Args:
            checkpoint_path : path to ScalePatchNet *_weights.pt
            patchSize       : must match the checkpoint's patchSize
            outFeatures     : number of disease classes (default 14)
            device          : 'cpu' or 'cuda'

        Returns:
            FiLMPatchNet on `device`
        """
