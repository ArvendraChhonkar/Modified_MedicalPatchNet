...
def fetchImg(line,size):
    ...
    coord = generate_coord_grid(scaledImg.shape[1], scaledImg.shape[2])
    scaledImg = torch.cat((scaledImg, coord), dim=0)  # Concatenate the image and coordinate grid along the channel dimension
